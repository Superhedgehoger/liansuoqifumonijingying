...
结合当前仓库实现，建议把外部能力按“先收益、后复杂”落地：

1) 先接选址优化（`spopt`/`maximum-coverage-location`）
- 目标：给出“下一批开店推荐”与覆盖率/收益提升估算

2) 再接路网可达（`osmnx + networkx`）
- 目标：把站点服务范围从“直线半径”升级为“通勤时间圈”

3) 最后接行为仿真（`mesa`）
- 目标：处理竞品分流、活动引流、口碑扩散等非线性变化

4) 事件与经营策略联动（参考 `OpenTTD` 的长期可玩性思路）
- 目标：事件不仅“打击”，还应允许运营动作“对冲”与恢复
