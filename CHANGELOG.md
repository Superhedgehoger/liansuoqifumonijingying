...
- 修复删除仍有关联门店的站点时产生孤立引用和保存失败的问题
- 后端以 HTTP 409 提供稳定的完整性保护，前端在单个/批量删除前给出关联门店清单
- 增加站点引用保护的回归测试与 Python/前端持续集成
- 移除不存在的 `/index.css` 引用，消除前端构建警告

## 0.8.0

- 建立正式的 `simgame.state` 数据模型版本，存档信封同时保留旧客户端可读的顶层 `version`
- 新增 0.7.x 存档和早期裸 `GameState` JSON 的兼容迁移，并拒绝误读未来版本存档
- 新增站点/门店/服务/库存引用一致性校验
- 新增存档往返、旧版融资字段迁移、未来版本拒绝、确定性模拟与订单不变式回归测试
